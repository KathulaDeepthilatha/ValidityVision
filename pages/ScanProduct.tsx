    ...
    <div className="flex-1 flex flex-col h-full bg-background-light dark:bg-background-dark overflow-hidden relative">
      <header className="w-full px-10 py-8 flex justify-between items-end shrink-0 z-20 relative animate-fade-in">
        ...
        <div className="flex items-center gap-4">
            ...
            <button className="bg-white dark:bg-surface-card-dark hover:bg-slate-50 dark:hover:bg-slate-700 text-text-secondary-light dark:text-text-secondary-dark hover:text-primary dark:hover:text-primary border border-slate-200 dark:border-slate-700 shadow-sm rounded-full p-3 relative transition-all hover:scale-110 active:scale-95 group">
                <span className="absolute top-2.5 right-3 w-2.5 h-2.5 bg-danger rounded-full animate-pulse border-2 border-white dark:border-surface-card-dark z-10"></span>
      ...
      <div className="flex-1 overflow-y-auto px-10 pb-10 custom-scrollbar z-10 relative">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-full max-h-[800px]">
            <div className="lg:col-span-2 flex flex-col gap-6 animate-fade-in" style={{ animationDelay: '200ms' }}>
                <div className="flex-1 relative bg-slate-900 rounded-[32px] overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-700 group cursor-pointer" onClick={handleScan}>
                    ...
                    <div className="absolute top-6 left-6 z-30">
                        <div className="flex items-center gap-2 bg-black/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/10">
                            ...
                            <span className="text-xs font-bold text-white uppercase tracking-wider">Analyzing Input</span>
                        </div>
                    </div>
                    
                    <div className="absolute top-6 right-6 z-30 flex gap-2">
                        <div className="w-10 h-10 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center text-white border border-white/10 hover:bg-white/20 cursor-pointer transition-colors">
                    ...
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 border border-white/30 rounded-2xl flex items-center justify-center z-10">
                        ...
                        <span className="absolute -bottom-8 text-white/70 text-xs font-medium tracking-wide">Align product here</span>
                    </div>
                </div>

                <div className="grid grid-cols-2 gap-4">
                    <button className="group relative flex items-center justify-center gap-3 py-5 rounded-[24px] bg-white dark:bg-surface-card-dark border border-slate-200 dark:border-slate-700 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all overflow-hidden">
                        ...
                        <span className="material-symbols-outlined text-3xl text-primary group-hover:scale-110 group-hover:rotate-12 transition-transform duration-300">add_photo_alternate</span>
                        <div className="flex flex-col items-start">
                            <span className="text-text-main-light dark:text-text-main-dark font-bold text-lg">Upload Image</span>
                            <span className="text-text-secondary-light dark:text-text-secondary-dark text-xs">From Gallery</span>
                        </div>
                    </button>
                    <button onClick={handleScan} className="group relative flex items-center justify-center gap-3 py-5 rounded-[24px] bg-gradient-to-br from-primary to-primary-dark text-white shadow-lg shadow-primary/30 hover:shadow-primary/50 hover:-translate-y-1 transition-all overflow-hidden">
                        <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
                        <div className="p-2 bg-white/20 rounded-full group-hover:bg-white/30 transition-colors">
                            <span className="material-symbols-outlined text-3xl group-hover:scale-110 transition-transform">camera</span>
                        </div>
                        <div className="flex flex-col items-start z-10">
                            <span className="font-bold text-lg">Take Photo</span>
                            <span className="text-primary-soft text-xs">Instant Scan</span>
                        </div>
                    </button>
                </div>
            </div>
            ...
            <div className="flex flex-col gap-6 animate-slide-in" style={{ animationDelay: '400ms' }}>
                <div className="p-1 bg-surface-card-light dark:bg-surface-card-dark rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex">
                    <button className="flex-1 py-2.5 rounded-xl text-sm font-bold bg-primary text-white shadow-md transition-all">Single</button>
                    <button className="flex-1 py-2.5 rounded-xl text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark hover:text-text-main-light dark:hover:text-text-main-dark transition-all">Batch</button>
                    <button className="flex-1 py-2.5 rounded-xl text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark hover:text-text-main-light dark:hover:text-text-main-dark transition-all">Barcode</button>
                </div>
